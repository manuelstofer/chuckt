...
(function(){
  ...
  if (typeof module !== 'undefined' && module.exports) {
    epixa = module.exports;
  } else {
    epixa = window.epixa || (window.epixa = {});
  }
  ...
  /**
   * Convenience method for applying chuckt event functionality to a
   * SockJS-node server connection.
   *
   * @param conn
   * @returns {ChuckT}
   */
  epixa.chucktifySockJSNode = function (conn) {
    var chuckt = new ChuckT({ send: conn.write.bind(conn)});
    conn.on('data', chuckt.process.bind(chuckt));
    conn.on('close', chuckt.removeListeners.bind(chuckt));
    return chuckt;
  };
  ...
  /**
   * Adds the given callback as listener for any event
   *
   * @param callback
   */
  ChuckT.prototype.any = function(callback) {
  ...
  ChuckT.prototype.process = function(message) {
    ...
    if (typeof chuckt.callbackid !== 'undefined'
      && typeof chuckt.event == 'undefined') {
      this.callbacks.use(chuckt.callbackid, chuckt.args);
      return;
    }

    if (typeof chuckt.callbackid !== 'undefined') {
      var socket = this.socket, serialize = this.serialize;
      chuckt.args.push(function(){
        return socket.send(serialize({
    ...
    if (typeof chuckt.event === 'string') {
      for (var i in this.anyListeners) {
        this.anyListeners[i].apply(this, [chuckt.event].concat(chuckt.args));
      }
