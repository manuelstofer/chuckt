...
(function(){
   ...
   * and onopen callback on the given socket, so this should only be used if
   * the only messages you expect to receive through the socket are generated
   * by a chuckt server implementation.
   *
   * @param socket
   */
  epixa.chucktify = function(socket) {
    ...
    if (chuckt.isConnecting()) {
      socket.onopen = function() {
        while (chuckt.connectEventBuffer.length) {
          chuckt.socket.send(chuckt.connectEventBuffer.shift());
        }
      };
    }
    return chuckt;
  ...
  ChuckT.prototype.emit = function(event) {
    ...
    if (this.isConnecting()) {
      this.connectEventBuffer.push(this.serialize(data));
    } else {
  ...
  /**
   * Checks if the the socket if the sockets ready state is connecting
   *
   * @returns {boolean}
   */
  ChuckT.prototype.isConnecting = function() {
